...
def get_predict():
    ...
    time_p = time.process_time() - time_start
    return {"result": result, "time": time_p}


@app.route("/trainscore")
def train_score(model="xgboost"):
    trainscore = train_and_persist(model=model)

    return {"train_score": trainscore}
